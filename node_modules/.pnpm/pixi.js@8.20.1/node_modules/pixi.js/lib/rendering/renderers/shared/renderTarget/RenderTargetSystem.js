...
class RenderTargetSystem {
  constructor(renderer) {
    ...
    this._bindFrame = new Rectangle.Rectangle();
  ...
  bind(surfaceOrOptions, clear = true, clearColor, frame, mipLevel = 0, layer = 0, flipY) {
    ...
    if ("target" in surfaceOrOptions) {
      ...
      deprecation.deprecation("8.20.0", "RenderTargetSystem.bind: positional arguments are deprecated, please use an options object instead: bind({ target, clear, clearColor, frame, mipLevel, layer, flipY })");
  ...
  get frontFaceInverted() {
    ...
    const glInherentFlip = this._renderer.type === types.RendererType.WEBGL && !renderTarget.isRoot;
  ...
  push(surfaceOrOptions, clear = _const.CLEAR.ALL, clearColor, frame, mipLevel = 0, layer = 0, flipY) {
    let options;
    if ("target" in surfaceOrOptions) {
      options = surfaceOrOptions;
    } else {
      deprecation.deprecation("8.20.0", "RenderTargetSystem.push: positional arguments are deprecated, please use an options object instead: push({ target, clear, clearColor, frame, mipLevel, layer, flipY })");
      options = { target: surfaceOrOptions, clear, clearColor, frame, mipLevel, layer, flipY };
    }
  ...
  copyDepthTexture(source, destination, originSrc, size, originDest = { x: 0, y: 0 }) {
    ...
    if (!sourceRenderTarget.depthStencilAttachment) {
      warn.warn("[RenderTargetSystem] copyDepthTexture: the source render target has no depth attachment to copy from");
      return;
    }
    const destSource = destination.source;
    if (!destSource.format.includes("depth") && !destSource.format.includes("stencil")) {
      warn.warn(`[RenderTargetSystem] copyDepthTexture: the destination texture must have a depth/stencil format (got '${destSource.format}')`);
      return;
    }
  ...
  ensureDepthStencil() {
    if (!this.renderTarget.stencil) {
      if (this.renderTarget.depthStencilTexture) {
        warn.warn(`[RenderTargetSystem] a stencil mask is being used, but the render target's depthStencilTexture format '${this.renderTarget.depthStencilTexture.format}' has no stencil aspect, so masking cannot work here. Use a 'depth24plus-stencil8' texture instead.`);
        return;
      }
  ...
  _initRenderTarget(renderSurface) {
    ...
    if (renderSurface instanceof RenderTarget.RenderTarget) {
      ...
      renderTarget = isDepthStencil ? new RenderTarget.RenderTarget({ colorTextures: 0, depthStencilTexture: renderSurface }) : new RenderTarget.RenderTarget({ colorTextures: [renderSurface] });
